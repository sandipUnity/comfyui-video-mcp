...
`workflow_catalog.auto_template_workflow()` converts a raw ComfyUI **API-format export** into a template: prompts are found by following the sampler's positive/negative links (with a CLIPTextEncode fallback), dimensions/frames/fps also via `PrimitiveInt` `_meta` titles, seeds/prefix/input-image by field name. The UI exposes this as an upload box. `pipeline/model_catalog.py` detects model-loader slots in a template (generic: any string input with a model-file extension) and lists installed server models per slot from `/object_info` (`comfyui_client.get_object_info()`; supports both classic `[[options], cfg]` and v3 `["COMBO", {options}]` specs). Per-slot overrides persist in `ProjectState.model_overrides_t2i/_i2v` and are applied by `apply_model_overrides()` after `fill_workflow()` (so a user pick beats the config default).

Legacy templates carrying sampler/model placeholders `fill_workflow()` can't natively fill (`{{CFG}}`, `{{STEPS}}`, `{{CHECKPOINT}}`, `{{TEXT_ENCODER}}`, `{{VAE}}`, `{{MOTION_MODULE}}`, `{{CAMERA_LORA}}`) become compatible via `workflow_catalog.template_defaults()`, which resolves those tokens from the **first** `config.yaml` `models:` entry whose `workflow:` key matches the template (`STEPS`/`CFG` fall back to `pipeline: default_steps/default_cfg`). Engines pass `extra=template_defaults(template)` into `fill_workflow()`, which routes numerics as bare pre-parse replacements and strings as post-parse injection (model filenames can contain backslashes — never raw-replace them into JSON). `fill_workflow()` raises if any `{{TOKEN}}` survives, so an incompatible template can't silently queue a literal placeholder. Templates whose calibrated values are baked into the JSON (`wan22_lightx2v_api`) intentionally have **no** extra placeholders → `template_defaults()` returns `{}`, protecting the do-not-tune values.
...
The same prompt text (built by `pipeline/prompt_builder.py`) feeds four interchangeable backends, parsed back by `pipeline/response_parser.py`. Each scene carries a `character_presence` field (`featured`/`background`/`none`, derived from `focus` via `_derive_presence_from_focus`, user-editable in the scene breakdown UI) that gates whether the locked character description is injected. **And a `focus`/`focus_subject` pair** (one of `subject`/`establishing`/`secondary`/`object`/`detail`/`phenomenon`/`reaction` plus a concrete frameable noun) that defines what the shot is *about* — the deterministic mechanical engine (`story_generator._enforce_focus_variety`) caps character scenes at ~45% and guarantees ≥30% when a character exists, so videos are never character-centric and never zero-character. Plus a storytelling triplet borrowed from OpenMontage's `scene_plan` schema (concepts only, AGPL not copied): `narrative_role` enum (`establish_context`, `build_tension`, `deliver_payload`, `resolution`, …), free-text `shot_intent`, and a `hero_moment` boolean that feeds the compositing layer (`montage.build_edit_decisions` / `compile_from_edit_decisions`) — hero scenes get extra hold time and cut transitions, subject scenes duck the music. Priority: **MCP worker** (this Claude Code session, free) → **Copy-Paste** (manual, any chatbot) → **Mechanical** (offline templates, fully local) → **API key** (`ANTHROPIC_API_KEY`, paid, lowest priority by design — the user is on a Max plan). Stages that use AI: story options, character description, scene prompts. All other steps are deterministic.
